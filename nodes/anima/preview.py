...
hover wipe (not yet built — `js/anima/` is a later slice), and OWNS SAVING
(contract: docs/generator-design.md §2/§7/§7a). Terminal, `OUTPUT_NODE =
True`: a graph with no Preview wired runs nothing at all, since the
Generator itself isn't an output node (design doc §2).
...
filename tokens); this class only wires up `INPUT_TYPES` and calls them.
"""
from __future__ import annotations

from ._preview_helpers import extract_seed_from_prompt, resolve_save_stages, save_images

CATEGORY = "AnimaFlow/Anima"


class AnimaPreview:
    """`Anima Preview` — terminal node: compares (not yet, pending
    `js/anima/`) and saves. `image_a`/`image_b`/`image_c` are all optional so
    "I don't want preview" is expressed by leaving them unwired — the whole
    Generator then does nothing at all, which is intended (design doc §2)."""
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                "image_a": ("IMAGE", {"tooltip": "First image to compare/save -- by convention (until js/anima/ lands) the Generator's image_base."}),
                "image_b": ("IMAGE", {"tooltip": "Second image to compare/save -- by convention (until js/anima/ lands) the Generator's image_mid."}),
                "image_c": ("IMAGE", {"tooltip": "Third image to compare/save -- by convention (until js/anima/ lands) the Generator's image."}),
            },
    ...
    def preview(self, preview_state="{}", image_a=None, image_b=None, image_c=None, prompt=None, extra_pnginfo=None):
        ...
        if isinstance(save_settings, dict) and save_settings.get("enabled", True):
            stages_to_save = resolve_save_stages(save_settings, compare_settings, wired)
            seed = extract_seed_from_prompt(prompt)
            ui_images = save_images(
                wired=wired, stages_to_save=stages_to_save, preview_settings=settings,
