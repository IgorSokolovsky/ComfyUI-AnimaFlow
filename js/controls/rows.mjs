 ...
 * `slotLabelOwned`.
 *
 * ## Layering (docs/control-panel-design.md §6a, 2026-07-29)
 *
 * The genuinely pure, no-Controls-shape maths that used to live in this file
 * (`rangeOf`/`clampNumeric`/`decimalsOf`/`numericPercent`/`formatNumericValue`/
 * `clampSeedString`/`randomSeedString`/`applyAfterGenerate`/`AFTER_MODES`/
 * `getComboOptions`) now lives in `js/shared/field_logic.mjs` — this file
 * imports it back and re-exports every name verbatim so no call site
 * anywhere in the pack had to change. Everything that's still HERE is either
 * genuinely Controls-shaped (the row catalog, slot/state bookkeeping, "auto"
 * row resolution — all keyed off `KIND_META`/`row.kind`/`row.slot`) or a
 * judgement call documented at its own definition site. See
 * `.claude/skills/animaflow-shared-fields/SKILL.md` for the three-layer
 * picture this split is part of.
 *
...
import {
  ...
  formatNumericValue,
  getComboOptions,
} from "../shared/field_logic.mjs";

// Re-exported verbatim -- see this module's own top doc comment ("Layering")
// for why these live in `js/shared/field_logic.mjs` now. No call site in
// either track changes: `import { clampSeedString } from "./rows.mjs"` still
...
// re-exported above) -- AFTER_LETTER stays here, it's a Controls-only
// display concern (the row's own mode-letter button, `render.mjs`'s
// `refs.modeBtn`), not shared maths.
...
// clampSeedString/randomSeedString/AFTER_MODES/applyAfterGenerate and
// decimalsOf/rangeOf/clampNumeric all now live in
// `js/shared/field_logic.mjs` (imported + re-exported at the top of this
// file) -- see this module's own top doc comment ("Layering"). `usefulRange`
// below stays here: it's a Controls-only heuristic used exclusively by
// `resolveAutoKind` (further down this file) when picking a draggable range
// for a freshly-resolved "auto" row, and nothing outside this track ever
// imports it.

// ---------------------------------------------------------------------------
// Numeric (int/float) range maths -- Controls-only heuristic
// ---------------------------------------------------------------------------
...
// formatNumericValue/numericPercent/getComboOptions all now live in
// `js/shared/field_logic.mjs` (imported + re-exported at the top of this
// file) -- see this module's own top doc comment ("Layering").
